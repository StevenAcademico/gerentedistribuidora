...
$('#dni_cliente').keyup(function(e) {
  ...
  var cl = $(this).val();
  $.ajax({
    url: url_static + "cliente/leer/"+cl,
    type: "POST",
    async: true,
    success: function(response) {
...
$('#form_new_cliente_venta').submit(function(e) {
  e.preventDefault();
    let form = $("#form_new_cliente_venta");
    ...
    form.validate({
        ...
        messages:{
          ...
          "tel_cliente":{
              required: "* Campo Requerido",
              minlength: "* Debe tener minimo 6 digitos",
              maxlength: "* Debe tener maximo 9 digitos"
          },
          "dir_cliente":{
              required: "* Campo Requerido",
          }
    ...
    if(form.valid()){
      $.ajax({
        url: url_static + "cliente/guardar",
        type: "POST",
        async: true,
...
$('#txt_cod_producto').keyup(function(e) {
  ...
  if (productos != '') {
    $.ajax({
      url: url_static + "producto/leer/"+productos,
      type: "POST",
      async: true,
      success: function(response){
        html = "";
        ...
        if(data.length > 0) {
          data.forEach(element => {
              html += `<option value="${element.codproducto}">${element.nombre}</option>`;
            });  
            $("#listamodelos").html(html);
        }        
      },
    });
...
$("#txt_cod_producto").on('input',function(){
  var val = document.getElementById("txt_cod_producto").value;
  var opts = document.getElementById('listamodelos').childNodes;
  for (var i = 0; i < opts.length; i++) {
    if (opts[i].value === val) {
      let value = opts[i].value;      
      $.ajax({
        url: url_static + "producto/leerId/"+value,
        type: "POST",
        async: true,
        success: function(response){
          if(response == null) {
            ...
            let detalles = info.detalles;
            let htmlCombo = "<select onchange='carga_detalle_producto(event);' id='cboPre' class='form-control'>";
            detalles.forEach(element => {
              htmlCombo += `<option data-stock='${element.stock}' data-precio='${element.precio}' data-codproducto='${element.codproducto}' value = '${element.idpresentacion}'>${element.presentacion}</option>`;
            });
            htmlCombo += "</select>";
            ...
            $("#cboPresentaciones").html(htmlCombo);            
            $("#cboPre").trigger("change");
            $('#txt_descripcion').html(producto.nombre);            
...
/*$('#txt_cod_producto').change(function(e) {
  e.preventDefault();
  var productos = $(this).val();
  if (productos == "") {
    $('#txt_descripcion').html('-');
...
$('#add_product_venta').click(function(e) {
  ...
  if ($('#txt_cant_producto').val() > 0) {
    ...
    var presentacion = $("#cboPre").val();
    var action = 'addProductoDetalle';
    $.ajax({
      url: url_static + "consulta/"+action+"/"+codproducto+"/"+cantidad + "/"+presentacion,
      type: 'POST',
      async: true,
      success: function(response) {
...
$('#btn_anular_venta').click(function(e) {
  ...
  if (rows > 0) {
    ...
    $.ajax({
      url: url_static + "consulta/"+action,
      type: 'POST',
      async: true,
      data: {action:action},
      success: function(response) {
...
$('#btn_facturar_venta').click(function(e) {
  ...
  var rows = $('#detalle_venta tr').length;
  if (rows > 0) {
    var codcliente = $('#idcliente').val() == '' ? 1 : $('#idcliente').val();
    $.ajax({
      url: url_static + "factura/guardar",
      type: 'POST',
      async: true,
      ...
      if (response != "error") {
        ...
        Swal.fire("Sistema","Error Al generar Venta","error");
      }
      },
      error: function(error) {
...
function generarPDF(cliente,factura) {
  url = url_static +  'consulta/generarFactura/'+cliente+'/'+factura;
...
function del_product_detalle(correlativo) {
  ...
  $.ajax({
    url: url_static + "consulta/" + action+ "/" + id_detalle,
    type: "POST",
    async: true,
    success: function(response) {
        if (response != "error") {
